...
每个 launcher 顶部包含 `HCU_LAUNCHER_*` 元数据，`run.sh` 查询时只读取这些赋值，不会 source launcher，也不会触发 HCU 环境。新增 launcher 不需要修改中央注册文件。
...
同一个 Qwen3 Dense launcher 可以接收 Qwen3-1.7B 或 Qwen3-8B；具体权重由 `--model-path` 指定。模型目录的 `config.json` 用于做 family、Dense/MoE/VL 一致性检查。
...
如果 family/variant/backend/rollout 不能唯一确定 launcher，`run.sh` 会报错并列出候选，不会静默选择错误的 Actor 或 Rollout backend。

Dense、MoE 和 VL 的区分规则是：launcher 元数据优先；模型目录存在 `config.json` 时，用 `vision_config`、`model_type` 和 `num_experts`/`num_local_experts` 做一致性检查；`num_experts` 或 `num_local_experts` 只有大于 1 才判定为 MoE。
...
`--profile` 仍然表示 Ray/AReaL 运行环境 profile（`qwen`、`qwen35`、`glm5`、`base`），不是模型规模。参数量只属于模型目录及其 `config.json`，不属于 launcher identity。

## 新增模型必须做的事情

1. 创建 `run_<family>_<variant>_<actor_backend>_<rollout_backend>.sh`。
2. 在 launcher 顶部填写 `HCU_LAUNCHER_FAMILY`、`HCU_LAUNCHER_VARIANT`、`HCU_LAUNCHER_ACTOR_BACKEND`、`HCU_LAUNCHER_ROLLOUT_BACKEND` 和 `HCU_LAUNCHER_PROFILE`。
3. 不要把参数量、层数或具体 checkpoint 名称写入 launcher 文件名；如果拓扑确实不同，应增加有意义的 variant，而不是增加 `8b`、`30b` 等字段。
4. 对照 AReaL v1.0.4 `cli_args.py` 和对应官方 YAML，确认每个 Hydra key 存在。
5. 设计正确的 Actor/Rollout TP/DP/PP/EP，保留模型特有的 SGLang/VLLM 参数在对应 launcher 内。
6. 不修改 `run.sh` 注册模型；`run.sh` 会扫描 rollout-aware launcher 文件自动发现。
7. 执行 `bash -n`、`bash run.sh --list`、`--info`、`--backends` 和 `--check-fsdp`。
8. 首次只做 10~20 step smoke test；如果环境/profile/source 路径变化，重启 Ray 后再测试。
...
`--restart-ray` 只支持 `N_NODES=1`，会在训练前重建当前节点 Ray。多节点必须分别执行 `--ray-head`、`--ray-worker`，最后只在 head 节点启动训练。`--dry-run` 不会启动或重启 Ray。
